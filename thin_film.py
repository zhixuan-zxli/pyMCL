...
class ThinFilmRunner(Runner):
    ...
    def prepare(self) -> None:
        ...
        self.xi_c, self.xi_c_fluid = xi_c, xi_c_fluid
        ...
        self.h = 1 - np.exp(4.0*(xi_c_fluid-1))
        self.h *= 1.0 + 0.2 * np.cos(20*xi_c_fluid)
        self.h[-1] = -self.h[-2]
    ...
    def pre_step(self) -> None:
        ...
        # delta_h = np.linalg.norm(h[2:-1] - self.h[2:-1], ord=np.inf) / solp.dt
        # delta_g = np.linalg.norm(g[2:-1] - self.g[2:-1], ord=np.inf) / solp.dt
        ...
        #
        self.ax.clear()
        self.ax.plot(self.a * self.xi_c_fluid[2:], self.h[2:], '-')
...
if __name__ == "__main__":
    ...
    solp = SolverParameters(dt = 1/(1024*4*4), Te=1.0)
