...
def testFiniteDifference(xi_b: np.ndarray):
    ...
    Ca = 1.0
    for m in range(256):
        # fill the ghosts near the symmetric boundary at x=0
        h[0], h[1] = h[3], h[2]
        ...
        v_lo2 = np.zeros((n+1, ))
        v_lo2[:-1] = fdtab[:,0]
        C = sp.diags((v_diag, v_up1, v_up2, v_lo1, v_lo2), (0, 1, 2, -1, -2), (n+3, n+3), "csr")
        ...
        ax.clear()
        ax.plot(xi_c[2:-2], h[2:-2], '-')
        ax.set_title("t = {}".format((m+1)*dt)); 
        ax.set_ylim(-0.1, 0.5); # ax.axis("equal")
        pyplot.draw(); pyplot.pause(0.1)
        pass
