...
def printConvergenceTable(mesh_table, error_table):
    ...
    header_str = "\n{0: <20}".format("")
    for i in range(m-1):
        header_str += "{0: <16}{1: <16}".format(mesh_table[i], "rate")
    header_str += "{0: <16}".format(mesh_table[-1])
    print(header_str)
    # print each norm
    for (norm_type, error_list) in error_table.items():
        error_str = "{0: <20}".format(norm_type)
        for i in range(m-1):
            error_str += "{0:<16.2e}{1:<16.2f}".format(error_list[i], np.log2(error_list[i]/error_list[i+1]))
        error_str += "{0:<16.2e}".format(error_list[-1])
        print(error_str)
...
def testStokes(mesh_div):
    ...
    f = Expression(("-2*pi*pi*sin(2*pi*x[1])*(1-2*sin(pi*x[0]))*(1+2*sin(pi*x[0])) - pi*exp(pi*x[0])*cos(pi*x[1])", \
                        "-2*pi*pi*sin(2*pi*x[0])*(2*sin(pi*x[1])+1)*(2*sin(pi*x[1])-1) + pi*exp(pi*x[0])*sin(pi*x[1])"), degree=3)

    u_exact = Expression(("sin(pi*x[0])*sin(pi*x[0])*sin(2*pi*x[1])", "-sin(2*pi*x[0])*sin(pi*x[1])*sin(pi*x[1])"), degree=3)
    p_exact = Expression("exp(pi*x[0])*sin(pi*x[1])", degree=2)
    ...
    u_err = interpolate(u_exact, FuncSpaces["U"])
